...
function Patients({ goTo }) {
  ...
  let list = PATIENTS;
  if (filter !== "all") {
    if (filter === "primeira_consulta") {
      list = list.filter(p => p.status === "primeira_consulta" || p.status === "primeira_fase");
    } else {
  ...
  return (
    <>
      <TPageHead
        eyebrow={`${PATIENTS.length} pacientes ativos · ${ALERTS.length} a acompanhar`}
      ...
      <div className="patient-toolbar">
        <div className="search-input">
          <IconSearch size={14} />
          <input
            placeholder="Procurar por nome ou foco terapêutico…"
            value={search}
      ...
      {list.length === 0 ? (
        <div style={{ padding: 60, textAlign: "center", color: "var(--ink-3)" }}>
          <div style={{ fontFamily: "var(--font-serif)", fontSize: 22, color: "var(--ink-2)", marginBottom: 6 }}>
            Sem resultados
          </div>
          <p>Ajusta a procura ou remove filtros.</p>
        </div>
      ) : view === "table" ? (
        <PatientTable list={list} statusLabels={statusLabels} goTo={goTo} />
      ) : (
        <PatientCards list={list} statusLabels={statusLabels} goTo={goTo} />
      )}
    </>
  );
}

function PatientTable({ list, statusLabels, goTo }) {
  return (
    <div className="card flush">
      <div className="table-head" style={{ gridTemplateColumns: "2fr 1.4fr 1.2fr 0.8fr 1fr 0.4fr" }}>
        <span>Paciente</span>
        <span>Foco terapêutico</span>
        <span>Próxima sessão</span>
        <span>Risco</span>
        <span>Tendência humor</span>
        <span></span>
      </div>
      <div className="list">
        {list.map(p => (
          <div key={p.id} className="row patient-row" onClick={() => goTo("patient", p)} style={{ gridTemplateColumns: "2fr 1.4fr 1.2fr 0.8fr 1fr 0.4fr" }}>
            <div className="patient-cell">
              <div className="avatar avatar-sm">{p.initials}</div>
              <div style={{ minWidth: 0 }}>
                <div className="row-title">
                  {p.name}
                  {p.alert && <span className="alert-dot" title={p.alert} />}
                </div>
                <div className="row-sub">{p.sessions} sessões · {p.since} · <PatientStatus status={p.status} label={statusLabels[p.status]} /></div>
              </div>
            </div>
...
function PatientCards({ list, statusLabels, goTo }) {
  return (
      ...
      {list.map(p => (
        <article key={p.id} className="patient-card" onClick={() => goTo("patient", p)}>
          ...
          <div className="pc-meta">
            ...
            <div><span className="pc-meta-label">Estado</span><span className="pc-meta-value"><PatientStatus status={p.status} label={statusLabels[p.status]} /></span></div>
          ...
          {p.moodTrend.length > 1 && (
            <div className="pc-trend">
              <Spark data={p.moodTrend} />
              <span style={{ fontSize: 11, color: "var(--ink-3)" }}>últimas 7 semanas</span>
            </div>
          )}
          {p.alert && (
            <div className="pc-alert">
              <IconAlert size={12} /> {p.alert}
            </div>
          )}
        </article>
      ))}
...
function Spark({ data }) {
  ...
  const color = trendUp ? "var(--status-done)" : last < first ? "var(--status-overdue)" : "var(--ink-3)";
