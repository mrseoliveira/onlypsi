// Therapist questionnaires — library + assignment overview

function TherapistQuestionnaires({ goTo }) {
  ...
  // Quick stats
  const totalAssigned = ACTIVITY.filter(a => a.type === "form_assigned" || a.type === "form_done").length + 47;
  ...
  return (
    <>
      <TPageHead
        ...
        sub={`${QUESTIONNAIRE_LIBRARY.length} instrumentos disponíveis · ${totalAssigned} atribuições esta semana · ${responseRate}% taxa de resposta`}
        actions={[
          ...
          {[
            ...
            <article key={i} className="assignment-card">
              <div className={"assignment-status " + a.status}>
                {a.status === "respondido" ? <IconCheck size={12} /> :
                 a.status === "alert" ? <IconAlert size={12} /> :
                 a.status === "em_atraso" ? <IconClock size={12} /> :
                 <IconClock size={12} />}
              </div>
              <div style={{ minWidth: 0 }}>
                <div className="assignment-name">{a.name}</div>
                <div className="assignment-meta">
                  <span>{a.to}</span> · <span>{a.when}</span>
                </div>
              </div>
      ...
      <div className="card flush">
        ...
        <div className="list">
          {list.map(q => (
            <div key={q.id} className="row q-lib-row" style={{ gridTemplateColumns: "2.2fr 1fr 1fr 0.8fr 0.8fr 0.8fr" }}>
              <div className="patient-cell">
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  <div className="row-sub">
                    {q.type === "Validado" ? "Instrumento validado" : "Próprio do terapeuta"} · {q.category}
                  </div>
                </div>
              </div>
...
Object.assign(window, { TherapistQuestionnaires });
