// Questionnaires list — to do + history with filter tabs

function Questionnaires({ goTo }) {
  const [tab, setTab] = React.useState("pending");
  const [filter, setFilter] = React.useState("all");

  const pending = PENDING_QUESTIONNAIRES;
  const history = HISTORY_QUESTIONNAIRES;

  const tabs = [
    { id: "pending", label: "A responder", count: pending.length },
  ...
  const filteredPending = filter === "all" ? pending : pending.filter(q => q.type === filter);
  const filteredHistory = filter === "all" ? history : history.filter(q => q.type === filter);
  ...
  return (
    <>
      <PageHead
        ...
        sub="Estes instrumentos ajudam a tua terapeuta a acompanhar o teu progresso e a preparar cada sessão. Tudo o que respondes é confidencial."
      />
      ...
      {tab === "pending" ? (
        <PendingList items={filteredPending} goTo={goTo} />
      ) : (
...
function QGrid({ items, goTo }) {
  return (
    <div className="qgrid">
      {items.map(q => <QCard key={q.id} q={q} onOpen={() => goTo("fill", q)} />)}
    </div>
  );
}
  ...
  return (
    <article className="qcard" onClick={onOpen}>
      <header className="qcard-head">
        <div className={"pq-marker " + q.status}>
...
Object.assign(window, { Questionnaires, formatDate });
