// Fill Questionnaire — Likert scale answering experience

function FillQuestionnaire({ goTo, questionnaire }) {
  const q = questionnaire || ACTIVE_QUESTIONNAIRE;
  // For history-item it might not be a full questionnaire — fall back
  ...
  const setAnswer = (id, v) => {
    ...
    setTimeout(() => {
      const next = items.find(it => it.id !== id && !(answers[it.id] != null || (id === it.id)));
      // pick first unanswered after this index
  ...
  if (submitted) {
    return <SubmittedScreen goTo={goTo} name={name} answered={answered} total={total} />;
  }

  return (
    <div className="fill-wrap" ref={containerRef}>
      {/* Sticky progress bar */}
      ...
      <div className="fill-canvas">
        ...
        <div className="fill-submit">
          <div>
            ...
            <p style={{ color: "var(--ink-3)", margin: "4px 0 0" }}>
              As tuas respostas são enviadas à tua terapeuta antes da sessão.
            </p>
          </div>
          <button
      ...
      <div className="qitem-body">
        ...
        <div className="likert">
          {options.map(opt => (
            <button
              key={opt.value}
              className={"likert-btn" + (value === opt.value ? " on" : "")}
...
function SubmittedScreen({ goTo, name, answered, total }) {
  return (
    <div className="fill-done">
      <div className="fill-done-card">
        <div className="fill-done-icon">
          <IconCheck size={36} />
        </div>
        ...
        <p style={{ color: "var(--ink-2)", fontSize: 16, maxWidth: 480, margin: "0 auto 28px" }}>
          As tuas {answered} respostas ao <strong>{name}</strong> foram enviadas para a Dra. Ana Sofia Martins. Vai estar tudo preparado para a sessão de hoje às 16:00.
        </p>
...
Object.assign(window, { FillQuestionnaire });
